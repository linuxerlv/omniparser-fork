...
from omnitool_ui.loop import (
    APIProvider,
    sampling_loop_sync,
)
from omnitool_ui.tools import ToolResult
...
def process_input(user_input, state):
    ...
    for loop_msg in sampling_loop_sync(
        ...
        output_callback=partial(
            chatbot_output_callback, chatbot_state=state["chatbot_messages"], hide_images=False
        ),
...
def get_file_viewer_html(file_path=None):
    ...
    if file_type == "image":
        ...
        file_extension in [".txt", ".py", ".js", ".html", ".css", ".json", ".md", ".csv"]
        or file_type == "text"
    ):
        ...
        try:
            ...
            return f'<div class="error-message">Error displaying PDF: {e!s}</div>'
...
def detect_new_files(state):
    ...
    print(
        f"Detected {new_files_count} new files. Total files in state: {len(state['uploaded_files'])}"
    )
...
with gr.Blocks(theme=gr.themes.Default()) as demo:
    ...
    if header_image:
        gr.HTML(
            f'<img src="{header_image}" alt="OmniTool Header" width="100%">',
            elem_classes="no-padding",
        )
        gr.HTML(
            '<h1 style="text-align: center; font-weight: normal; margin-bottom: 20px;">Omni<span style="font-weight: bold;">Tool</span></h1>'
        )
    else:
            ...
            with gr.Column():
                with gr.Row():
                    upload_button = gr.Button(
                        "Upload Files", variant="primary", elem_classes="primary-button"
                    )
                    refresh_button = gr.Button(
                        "Refresh Files", variant="secondary", elem_classes="secondary-button"
                    )

        with gr.Row():
            # Initialize file choices as an empty list
            view_file_dropdown = gr.Dropdown(
                label="View File", choices=[], interactive=True, container=True
            )
    ...
    with gr.Row():
        ...
        with gr.Column(scale=1, min_width=50):
            submit_button = gr.Button(
                value="Send", variant="primary", elem_classes="primary-button"
            )
        with gr.Column(scale=1, min_width=50):
            stop_button = gr.Button(
                value="Stop", variant="secondary", elem_classes="secondary-button"
            )

    with gr.Row():
        with gr.Column(scale=2):
            chatbot = gr.Chatbot(
                label="Chatbot History", autoscroll=True, height=580, avatar_images=("👤", "🤖")
            )
        with gr.Column(scale=3):
            display_area = gr.HTML(get_file_viewer_html(), elem_classes="no-padding")
    ...
    def update_model(model_selection, state):
        ...
        provider_update = gr.update(
            choices=provider_choices, value=default_provider_value, interactive=provider_interactive
        )
        api_key_update = gr.update(placeholder=api_key_placeholder, value=state["api_key"])
    ...
    upload_button.click(
        fn=handle_file_upload, inputs=[file_upload, state], outputs=[view_file_dropdown]
    )

    # File viewing handlers
    view_file_dropdown.change(
        fn=view_file, inputs=[view_file_dropdown, view_toggle], outputs=[display_area]
    )
