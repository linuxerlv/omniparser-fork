...
from omnitool_ui.agent.llm_utils.groqclient import run_groq_interleaved
from omnitool_ui.agent.llm_utils.oaiclient import run_oai_interleaved
from omnitool_ui.agent.llm_utils.utils import is_image_path
...
class VLMAgent:
    ...
    def __call__(self, messages: list, parsed_screen: list[str, list, dict]):
        ...
        self.output_callback(
            f"LLM: {latency_vlm:.2f}s, OmniParser: {latency_omniparser:.2f}s", sender="bot"
        )
        ...
        if "Box ID" in vlm_response_json:
            try:
                bbox = parsed_screen["parsed_content_list"][int(vlm_response_json["Box ID"])][
                    "bbox"
                ]
                ...
                print(f"Error parsing: {vlm_response_json} ({exc})")
        self.output_callback(
